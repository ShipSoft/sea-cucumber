...
// GeoModel lengths are in mm; TGeo native unit is cm.
...
TGeoHMatrix toTGeo(const GeoTrf::Transform3D& g)
{
    ...
    Double_t tr[3] = { T.x() * kMmToCm, T.y() * kMmToCm, T.z() * kMmToCm };
    TGeoHMatrix m;
    m.SetRotation(rot);
    m.SetTranslation(tr);
    return m;
...
TGeoShape* makeBoolean(const char* kind,
                       const GeoShape* opA, const GeoShape* opB)
{
    ...
    if      (k == "union")        node = new TGeoUnion(a, b, mA, mB);
    else if (k == "subtraction")  node = new TGeoSubtraction(a, b, mA, mB);
    else /* intersection */       node = new TGeoIntersection(a, b, mA, mB);
...
TGeoShape* convertShape(const GeoShape* s, TGeoHMatrix& localShift)
{
    ...
    if (const auto* box = dynamic_cast<const GeoBox*>(s)) {
        return new TGeoBBox(box->getXHalfLength() * kMmToCm,
                            box->getYHalfLength() * kMmToCm,
                            box->getZHalfLength() * kMmToCm);
    }
    if (const auto* tube = dynamic_cast<const GeoTube*>(s)) {
        return new TGeoTube(tube->getRMin() * kMmToCm,
                            tube->getRMax() * kMmToCm,
                            tube->getZHalfLength() * kMmToCm);
    }
    if (const auto* tubs = dynamic_cast<const GeoTubs*>(s)) {
        ...
        return new TGeoTubeSeg(tubs->getRMin() * kMmToCm,
                               tubs->getRMax() * kMmToCm,
                               tubs->getZHalfLength() * kMmToCm,
                               sphi, sphi + dphi);
    }
    if (const auto* cons = dynamic_cast<const GeoCons*>(s)) {
        ...
        return new TGeoConeSeg(cons->getDZ() * kMmToCm,
                               cons->getRMin1() * kMmToCm,
                               cons->getRMax1() * kMmToCm,
                               cons->getRMin2() * kMmToCm,
                               cons->getRMax2() * kMmToCm,
                               sphi, sphi + dphi);
    }
    if (const auto* trd = dynamic_cast<const GeoTrd*>(s)) {
        return new TGeoTrd2(trd->getXHalfLength1() * kMmToCm,
                            trd->getXHalfLength2() * kMmToCm,
                            trd->getYHalfLength1() * kMmToCm,
                            trd->getYHalfLength2() * kMmToCm,
                            trd->getZHalfLength()  * kMmToCm);
    }
    if (const auto* para = dynamic_cast<const GeoPara*>(s)) {
        return new TGeoPara(para->getXHalfLength() * kMmToCm,
                            para->getYHalfLength() * kMmToCm,
                            para->getZHalfLength() * kMmToCm,
                            para->getAlpha() * kRadToDeg,
                            para->getTheta() * kRadToDeg,
                            para->getPhi()   * kRadToDeg);
    }
    if (const auto* trap = dynamic_cast<const GeoTrap*>(s)) {
        return new TGeoTrap(trap->getZHalfLength() * kMmToCm,
                            trap->getTheta() * kRadToDeg,
                            trap->getPhi()   * kRadToDeg,
                            trap->getDydzn() * kMmToCm,
                            trap->getDxdyndzn() * kMmToCm,
                            trap->getDxdypdzn() * kMmToCm,
                            trap->getAngleydzn() * kRadToDeg,
                            trap->getDydzp() * kMmToCm,
                            trap->getDxdyndzp() * kMmToCm,
                            trap->getDxdypdzp() * kMmToCm,
                            trap->getAngleydzp() * kRadToDeg);
    }
    if (const auto* eltu = dynamic_cast<const GeoEllipticalTube*>(s)) {
        return new TGeoEltu(eltu->getXHalfLength() * kMmToCm,
                            eltu->getYHalfLength() * kMmToCm,
                            eltu->getZHalfLength() * kMmToCm);
    }
    if (const auto* torus = dynamic_cast<const GeoTorus*>(s)) {
        ...
        return new TGeoTorus(torus->getRTor() * kMmToCm,
                             torus->getRMin() * kMmToCm,
                             torus->getRMax() * kMmToCm,
                             sphi, dphi);
    }
    if (const auto* pcon = dynamic_cast<const GeoPcon*>(s)) {
        ...
        for (unsigned i = 0; i < n; ++i)
            p->DefineSection((Int_t)i,
                             pcon->getZPlane(i)    * kMmToCm,
                             pcon->getRMinPlane(i) * kMmToCm,
                             pcon->getRMaxPlane(i) * kMmToCm);
        return p;
    }
    if (const auto* pgon = dynamic_cast<const GeoPgon*>(s)) {
        ...
        for (unsigned i = 0; i < n; ++i)
            p->DefineSection((Int_t)i,
                             pgon->getZPlane(i)    * kMmToCm,
                             pgon->getRMinPlane(i) * kMmToCm,
                             pgon->getRMaxPlane(i) * kMmToCm);
        return p;
    }
...
void walk(const GeoVPhysVol* vol,
          const GeoTrf::Transform3D& parentToWorld,
          int depth, WalkState& st)
{
    ...
    for (unsigned i = 0; i < nChild; ++i) {
        if (st.emitted >= st.opt.max_shapes) {
            if (!st.capped) {
                std::cerr << "[GeoModelLoader] hit max_shapes="
                          << st.opt.max_shapes << " -- stopping (tighten "
...
{
    if (!world)
        throw std::runtime_error("GeoModelLoader: null world volume");
    ...
    if (opt.verbose && st.emitted == 0) {
        std::cerr << "[GeoModelLoader] 0 shapes matched out of "
                  << st.visited << " volumes visited.\n"
        ...
        std::cout << "[GeoModelLoader] emitted " << st.emitted << " shapes ("
                  << st.visited << " volumes visited)\n";
    }
    return st.emitted;
}
    ...
    auto worldLink = [&]() {
        try {
            ...
            throw std::runtime_error(
                std::string("GeoModelLoader: buildGeoModel failed: ") + e.what());
        }
    }();
    if (!worldLink)
