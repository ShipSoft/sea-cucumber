...
TGeoShape* convertShape(const GeoShape* s, TGeoHMatrix& localShift)
{
    ...
    if (const auto* box = dynamic_cast<const GeoBox*>(s)) {
        return new TGeoBBox(box->getXHalfLength() * gLenScale,
                            box->getYHalfLength() * gLenScale,
                            box->getZHalfLength() * gLenScale);
    }
    if (const auto* tube = dynamic_cast<const GeoTube*>(s)) {
        return new TGeoTube(tube->getRMin() * gLenScale,
                            tube->getRMax() * gLenScale,
    ...
    if (const auto* tubs = dynamic_cast<const GeoTubs*>(s)) {
        ...
        return new TGeoTubeSeg(tubs->getRMin() * gLenScale,
                               tubs->getRMax() * gLenScale,
                               tubs->getZHalfLength() * gLenScale,
                               sphi, sphi + dphi);
    }
    if (const auto* cons = dynamic_cast<const GeoCons*>(s)) {
        ...
        return new TGeoConeSeg(cons->getDZ() * gLenScale,
                               cons->getRMin1() * gLenScale,
                               cons->getRMax1() * gLenScale,
                               cons->getRMin2() * gLenScale,
                               cons->getRMax2() * gLenScale,
                               sphi, sphi + dphi);
    }
    if (const auto* trd = dynamic_cast<const GeoTrd*>(s)) {
        return new TGeoTrd2(trd->getXHalfLength1() * gLenScale,
                            trd->getXHalfLength2() * gLenScale,
                            trd->getYHalfLength1() * gLenScale,
                            trd->getYHalfLength2() * gLenScale,
                            trd->getZHalfLength()  * gLenScale);
    }
    if (const auto* para = dynamic_cast<const GeoPara*>(s)) {
        return new TGeoPara(para->getXHalfLength() * gLenScale,
                            para->getYHalfLength() * gLenScale,
                            para->getZHalfLength() * gLenScale,
                            para->getAlpha() * kRadToDeg,
                            para->getTheta() * kRadToDeg,
                            para->getPhi()   * kRadToDeg);
    }
    if (const auto* trap = dynamic_cast<const GeoTrap*>(s)) {
        return new TGeoTrap(trap->getZHalfLength() * gLenScale,
                            trap->getTheta() * kRadToDeg,
                            trap->getPhi()   * kRadToDeg,
                            trap->getDydzn() * gLenScale,
                            trap->getDxdyndzn() * gLenScale,
                            trap->getDxdypdzn() * gLenScale,
                            trap->getAngleydzn() * kRadToDeg,
                            trap->getDydzp() * gLenScale,
                            trap->getDxdyndzp() * gLenScale,
                            trap->getDxdypdzp() * gLenScale,
                            trap->getAngleydzp() * kRadToDeg);
    }
    if (const auto* eltu = dynamic_cast<const GeoEllipticalTube*>(s)) {
        return new TGeoEltu(eltu->getXHalfLength() * gLenScale,
                            eltu->getYHalfLength() * gLenScale,
    ...
    if (const auto* torus = dynamic_cast<const GeoTorus*>(s)) {
        ...
        return new TGeoTorus(torus->getRTor() * gLenScale,
                             torus->getRMin() * gLenScale,
                             torus->getRMax() * gLenScale,
                             sphi, dphi);
    }
    if (const auto* pcon = dynamic_cast<const GeoPcon*>(s)) {
        ...
        for (unsigned i = 0; i < n; ++i)
            p->DefineSection((Int_t)i,
                             pcon->getZPlane(i)    * gLenScale,
                             pcon->getRMinPlane(i) * gLenScale,
                             pcon->getRMaxPlane(i) * gLenScale);
        return p;
    }
    if (const auto* pgon = dynamic_cast<const GeoPgon*>(s)) {
        ...
        for (unsigned i = 0; i < n; ++i)
            p->DefineSection((Int_t)i,
                             pgon->getZPlane(i)    * gLenScale,
                             pgon->getRMinPlane(i) * gLenScale,
                             pgon->getRMaxPlane(i) * gLenScale);
        return p;
    }
