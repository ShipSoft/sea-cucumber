...
//  simParticles() prefer the flat `sim_hits` / `sim_particles` fields and fall
//  back to `sim_result`'s bundled collections when the flat ones are absent,
//  so a file written with only `sim_result` still displays.
//
